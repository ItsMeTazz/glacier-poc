...
export default function ssBribeCreate() {
  ...
  const ssUpdated = async () => {
    ...
    const storePairs = stores.stableSwapStore.getStore('rewardpairs')
    console.log(storePairs,"storePairsstorePairsstorePairsstorePairsstorePairsstorePairs")
    const filterPair = storePairs.filter((item)=> {
      if(item.gauge.bribeAddress){
        return item;
      }
    });
    
    setAssetOptions(filteredStoreAssetOptions)
    setGaugeOptions(filterPair)

    if(filteredStoreAssetOptions.length > 0 && asset == null) {
      setAsset(filteredStoreAssetOptions[0])
    }

    if(filterPair.length > 0 && gauge == null) {
      setGauge(filterPair[0])
    }
  }
                  ...
                  <MenuItem key={option.id} value={option} style={{background:"black", border:"2px solid gray"}}>
                    <div className={ classes.menuOption }>
                      ...
                      <div>
                        ...
                        <Typography color='textSecondary' className={ classes.smallerText }>{ option?.stable ? 'Stable Pool' : 'Volatile Pool' }</Typography>
                      </div>
                    </div>
                  </MenuItem>
                )
