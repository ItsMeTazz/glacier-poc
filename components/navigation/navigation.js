  ...
  return (
    <SvgIcon viewBox="0 0 147.7 17.5" className={className}>
      <g>
        ...
        <circle fill="#FF01D6 " cx="32.4" cy="8.8" r="8.8"/>
...
function Navigation(props) {
  ...
  useEffect(() => {
    ...
    if(activePath.includes('bribe/create')) {
      setActive('bribe/create')
    }
  ...
  const renderSubNav = (title, link) => {
    ...
    return (
      <ToggleButton value={link} className={ classes.navButton } classes={{ selected: classes.testChange }}>
        <Typography variant="h1" className={ classes.subtitleText}>{title}</Typography>
      </ToggleButton>
    );
