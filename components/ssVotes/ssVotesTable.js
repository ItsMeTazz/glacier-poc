...
const PrettoSlider = withStyles({
  ...
  thumb: {
    height: 24,
    width: 24,
    background: "linear-gradient(180deg, #F4F1F7 0%, #ECE3F3 44.13%, #8F7D9D 86.26%, #F6ECFD 100%)",
  ...
  track: {
    background:"linear-gradient(90deg,cyan,blue,purple,magenta,red)",
    color:"none",
    height: 8,
  },
  rail: {
...
function descendingComparator(a, b, orderBy) {
  ...
  switch (orderBy) {
    case 'Bribes+Fees':
      ...
      if (BigNumber(b?.gauge?.tbvUSD).lt(a?.gauge?.tbvUSD)) {
        return -1;
      }
      if (BigNumber(b?.gauge?.tbvUSD).gt(a?.gauge?.tbvUSD)) {
        return 1;
      }
      return 0;
    ...
    case 'liquidity':
      ...
      if (BigNumber(b?.gauge?.aprUsd).lt(a?.gauge?.aprUsd)) {
        return -1;
      }
      if (BigNumber(b?.gauge?.aprUsd).gt(a?.gauge?.aprUsd)) {
        return 1;
      }
      return 0;
    ...
    case 'myVotes':
      if (BigNumber(b?.voteValue).lt(a?.voteValue)) {
        return -1;
      }
      if (BigNumber(b?.voteValue).gt(a?.voteValue)) {
        return 1;
      }
      return 0;
...
const useStyles = makeStyles((theme) => ({
  ...
  img1Logo: {
    ...
    left: '0px',
    top: '0px',
    border: '3px solid rgb(1,1,27)',//rgb(25, 33, 56)',
    borderRadius: '30px',
  },
  img2Logo: {
    position: 'absolute',
...
console.log({gauges,token})
//token = {id: '12', lockEnds: '1684972800', lockAmount: '36.224365355980226328', lockValue: '31.935281445425141864'}
  // gauges.map((item) => {
  //   let bf = 0.0;
  //   item.gauge.bribes.map((row, index) => {
  //     let rewardAmmount = row.rewardAmmount;
  //     let newRewardAmmount = parseFloat(rewardAmmount)
  //     let price = row.token.price;
  //     let newBF = parseFloat(price * newRewardAmmount)
  //     bf = bf + newBF;
  //   })
  //   item.bribesFees = bf;
  // })
  gauges = gauges.filter((item)=>{
    item.voteValue=defaultVotes.find((el) => el.address === item?.address)?.value || 0;
    return item;
  })
  ...
  return (
    ...
    <div className={classes.root}>
                ...
                return (
                  <TableRow key={row?.gauge?.address}>
                    ...
                    <div className={ classes.inlineEnd }>
                      ...
                      <Typography variant='h5' className={classes.textSpaced} color='textSecondary'>
                        {row?.token1?.symbol}
                          ...
                          {formatCurrency(row?.gauge.aprUsd)}%
                    ...
                    <div className={ classes.inlineEnd }>
                      ...
                      <Typography variant='h5' className={classes.textSpaced} color='textSecondary'>
                        {row?.token1?.symbol} */}
                          {/* ${formatCurrency(row?.bribesFees)} */}
                          ${formatCurrency(row?.gauge.tbvUSD)}
                    ...
                    <TableCell className={classes.cell} align="right">
                      {
                        row?.gauge?.bribes.map((bribe, idx) => {
                          ...
                          return (
                            formatCurrency(bribe.rewardAmmount) != 0 &&
                            <div className={classes.inlineEnd}>
                              
                               <Typography variant="h2" className={classes.textSpaced}>{formatCurrency(bribe.rewardAmmount)}</Typography>
      ...
      <TablePagination
        ...
        style={{border:"2px solid gray", borderTop:"none", background:"black"}}
      />
    </div>
  );
}
