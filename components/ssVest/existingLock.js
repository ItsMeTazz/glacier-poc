...
export default function existingLock({ nft, govToken, veToken }) {
  ...
  const updateLockAmount = (amount) => {
    ...
    tmpNFT.lockValue = BigNumber(tmpNFT.lockAmount).times(parseInt(dayToExpire)).div(90).toFixed(18)
  ...
  const updateLockDuration = (val) => {
    ...
    tmpNFT.lockValue = BigNumber(tmpNFT.lockAmount).times(parseInt(dayToExpire)).div(83).toFixed(18)
