...
class Store {
    ...
    dispatcher.register(
      function (payload) {
        switch (payload.type) {
          ...
          // LIQUIDITY
          case ACTIONS.CREATE_PAIR_AND_STAKFE:
  ...
  getBaseAsset = async (address, save, getBalance) => {
    try {
      ...
      const [symbol, decimals, name, balanceOf] =
        await this._tryGetBaseAssetDetails(web3, address, getBalance, account);
  ...
  _tryGetBaseAssetDetails = async (web3, address, getBalance, account) => {
    try {
      ...
      const baseAssetDetails = await this._getBaseAssetDetails(
        web3,
        multicall,
        address,
        getBalance,
        ...
        pairs.map(async (pair) => {
          try {
            ...
            const [totalSupply, reserves, balanceOf, claimable0, claimable1] =
              await this._tryGetPairBalances(web3, pair, account);
        ...
        ps.map(async (pair) => {
          try {
            if (
              ...
              pair.gauge.address !== ZERO_ADDRESS
            ) {
              const [totalSupply, gaugeBalance, gaugeWeight] =
                await this._tryGetGaugeBalances(
                  web3,
                  pair,
                  account,
                  gaugesContract
              ...
              if (pair.gauge.internal_bribe !== ZERO_ADDRESS) {
                const bribeContract = new web3.eth.Contract(
                  CONTRACTS.INT_BRIBE_ABI,
                  pair.gauge.internal_bribe
                );

                if (pair.gauge.bribesi) {
                  // add check for existence of bribe array
                  const bribes = await Promise.all(
  ...
  _tryGetPairBalances = async (web3, pair, account) => {
    try {
      const multicall = await stores.accountStore.getMulticall();
      const pairBalances = await this._getPairBalances(
        web3,
        multicall,
        pair,
        account
  ...
  _tryGetGaugeBalances = async (web3, pair, account, gaugesContract) => {
    try {
      const multicall = await stores.accountStore.getMulticall();
      const gaugeBalance = await this._getGaugeBalances(
        web3,
        multicall,
        pair,
        account,
  ...
  _getBaseAssetInfo = async (web3, account) => {
    try {
      ...
      const balanceOfs = await this._tryGetBalanceOfs(
        web3,
        baseAssets,
        account
      );
  ...
  _tryGetBalanceOfs = async (web3, baseAssets, account) => {
    try {
      ...
      const balanceOfs = await this._getBalanceOfs(
        web3,
        multicall,
        baseAssets,
        account
  ...
  _tryGetWhitelists = async (web3, baseAssets) => {
    try {
      ...
      try {
        const multicall = await stores.accountStore.getMulticall(true);
        const whitelists = await this._getWhitelists(
          web3,
          multicall,
          baseAssets
        );
  ...
  createPairDeposit = async (payload) => {
    ...
    try {
      ...
      const { token0, token1, amount0, amount1, isStable, slippage, token } =
        payload.content;
      let toki0 = token0.address;
      let toki1 = token1.address;
      if (token0.address === "AVAX") {
  ...
  quoteSwap = async (payload) => {
    try {
      ...
      if (
        (fromAsset.symbol == "USDC" && toAsset.symbol == "GLACIER") ||
        (fromAsset.symbol == "GLACIER" && toAsset.symbol == "GLACIER")
      ) {
        amountOuts.push({
          routes: [
            {
            ...
            pair.gauge.bribes.map(async (bribe) => {
              ...
              return {
                earned: BigNumber(earned)
                  .div(10 ** parseInt(bribe.token.decimals))
                  .toFixed(6),
              };
            })
          );
      ...
      if (tokenID) {
        ...
        filteredFeesBribe = FeesEarned.filter((pair) => {
          if (
            ...
            pair.gauge.feesEarned.length > 0
          ) {
            let shouldReturn = false;
  ...
  claimFeesBribes = async (payload) => {
    try {
      ...
      const sendTokens = [
        pair.gauge.feesEarned.map((bribe) => {
          return bribe.token.address;
        }),
      ];
      ...
      if (feesBribePairs.length > 0) {
        ...
        const claimPromise = new Promise((resolve, reject) => {
          context._callContractWait(
            ...
            gaugesContract,
            "claimFees",
            [sendGaugesFee, sendTokensFee, tokenID],
            account,
            gasPrice,
            null,
            null,
