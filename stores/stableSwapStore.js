...
import { AltRoute, ConnectedTvOutlined, ConnectingAirportsOutlined, ConstructionOutlined, ContactEmergencySharp } from "@mui/icons-material"
...
class Store {
      ...
      function (payload) {
        switch (payload.type) {
          ...
          case ACTIONS.CLAIM_BRIBE:
            ...
            break
          case ACTIONS.CLAIM_FEES_BRIBE:
            this.claimFeesBribes(payload)
            break
      ...
      if (pairAddress && pairAddress != ZERO_ADDRESS) {
        ...
        if (gaugeAddress !== ZERO_ADDRESS) {
          ...
          const [totalSupply, gaugeBalance, bribeAddress] = await Promise.all([
            gaugeContract.methods.totalSupply().call(),
            gaugeContract.methods.balanceOf(account.address).call(),
            gaugesContract.methods.rewards(gaugeAddress).call()
          ])
  ...
  _getRouteAssets = async () => {
    try {
      ...
      const response = await fetch(`https://glacierapi.herokuapp.com/api/routes`, {
  ...
  _getPairs = async () => {
    try {
     ...
     const response = await fetch(`https://glacierapi.herokuapp.com/api/pairs`, {
      ...
      const ps = await Promise.all(
        pairs.map(async (pair) => {
          try {
            ...
            pair.claimable0 = BigNumber(claimable0).div(10 ** pair.token0.decimals).toFixed(6)
            pair.claimable1 = BigNumber(claimable1).div(10 ** pair.token1.decimals).toFixed(6)
...
console.log('balanceOf:', balanceOf)
      ...
      const ps1 = await Promise.all(
        ps.map(async (pair) => {
          try {
            if (pair.gauge.bribeAddress && pair.gauge.address !== ZERO_ADDRESS) {
              const [totalSupply, gaugeBalance, gaugeWeight] = await this._tryGetGaugeBalances(web3, pair, account, gaugesContract)
    
              if (pair.gauge.internal_bribe !== ZERO_ADDRESS) {
                const bribeContract = new web3.eth.Contract(CONTRACTS.INT_BRIBE_ABI, pair.gauge.internal_bribe)
    
                if (pair.gauge.bribesi) { // add check for existence of bribe array
                  const bribes = await Promise.all(
                    pair.gauge.bribesi.map(async (bribe, idx) => {
    
                      const rewardRate = await bribeContract.methods.rewardRate(bribe.token.address).call()
    
                      bribe.rewardRate = BigNumber(rewardRate).div(10 ** bribe.token.decimals).toFixed(6)
                      bribe.rewardAmount = BigNumber(rewardRate).times(604800).div(10 ** bribe.token.decimals).toFixed(6)
              ...
              pair.gauge.reserve0 = pair.totalSupply > 0 ? BigNumber(pair.reserve0).times(pair.gauge.totalSupply).div(pair.totalSupply).toFixed(6) : '0'
              pair.gauge.reserve1 = pair.totalSupply > 0 ? BigNumber(pair.reserve1).times(pair.gauge.totalSupply).div(pair.totalSupply).toFixed(6) : '0'
            ...
            console.log('pairs2:', ps)
    
            console.log('EXCEPTION 2:', ex.message)
  ...
  _getPairBalances = async (web3, multicall, pair, account) => {
    try {
      ...
      console.log(pairContract, "this is the paircontract");
  ...
  _tryGetPairBalances = async (web3, pair, account) => {
    try {
      ...
      const pairBalances = await this._getPairBalances(web3, multicall, pair, account)
      console.log(pairBalances, "pairbalances");
      return pairBalances
    } catch (ex) {
      try {
         const multicall = await stores.accountStore.getMulticall(true)
  ...
  _tryGetBalanceOfs = async (web3, baseAssets, account) => {
    try {
      ...
      console.log("_tryGetBalanceOfs", balanceOfs)
      return balanceOfs
    } catch (ex) {
      try {
        const multicall = await stores.accountStore.getMulticall(true)
  ...
  createPairStake = async (payload) => {
    ...
    try {
      ...
      this.emitter.emit(ACTIONS.TX_ADDED, {
        title: `Create liquidity pool for ${token0.symbol}/${token1.symbol}`, type: 'Liquidity', verb: 'Liquidity Pool Created', transactions: [
          {
            uuid: allowance0TXID,
      ...
      this._callContractWait(web3, routerContract, func, params, account, gasPrice, null, null, depositTXID, async (err) => {
        ...
        this._callContractWait(web3, gaugesContract, 'createGauge', [pairFor], account, gasPrice, null, null, createGaugeTXID, async (err) => {
          ...
          // //alert("gaugeAddress"+gaugeAddress)
          // console.log({ gaugeAddress, pairFor })
          ...
          //   }
          // }
          // const stakeAllowance = await this._getStakeAllowance(web3, pair1, account)
          // //alert("stake Allow")
          // if (BigNumber(stakeAllowance).lt(BigNumber(balanceOf).div(10 ** pair.decimals).toFixed(6))) {
          //   this.emitter.emit(ACTIONS.TX_STATUS, {
          //     uuid: stakeAllowanceTXID,
  ...
  createPairDeposit = async (payload) => {
    ...
    try {
      ...
      const { token0, token1, amount0, amount1, isStable, slippage, token } = payload.content
      let toki0 = token0.address
      let toki1 = token1.address
      if (token0.address === 'AVAX') {
        toki0 = CONTRACTS.WFTM_ADDRESS
      ...
      this._callContractWait(web3, routerContract, func, params, account, gasPrice, null, null, depositTXID, async (err) => {
        ...
        console.log({ tok0, tok1, isStable })
        const pairFor = await factoryContract.methods.getPair(tok0, tok1, isStable).call()
        // SUBMIT CREATE GAUGE TRANSACTION
        const gaugesContract = new web3.eth.Contract(CONTRACTS.VOTER_ABI, CONTRACTS.VOTER_ADDRESS)
        this._callContractWait(web3, gaugesContract, 'createGauge', [pairFor], account, gasPrice, null, null, createGaugeTXID, async (err) => {
  ...
  updatePairsCall = async (web3, account) => {
    try {
      ...
      const response = await fetch(`https://glacierapi.herokuapp.com/api/updatePairs`, {
        method: 'get',
        headers: {
          'Authorization': `Basic ${process.env.NEXT_PUBLIC_API_TOKEN}`,
        }
  ...
  addLiquidity = async (payload) => {
    try {
      ...
      this.emitter.emit(ACTIONS.TX_ADDED, {
        title: `Add liquidity to ${pair.symbol}`, verb: 'Liquidity Added', type: 'Liquidity', transactions: [
          {
            uuid: allowance0TXID,
            description: `Checking your ${token0.symbol} allowance`,
            status: 'WAITING'
      ...
      let func = 'addLiquidity'
      let params = [token0.address, token1.address, pair.stable, sendAmount0, sendAmount1, sendAmount0Min, sendAmount1Min, account.address, deadline]
      let sendValue = null

      if (token0.address === 'AVAX') {
        func = 'addLiquidityAVAX'
        params = [token1.address, pair.stable, sendAmount1, sendAmount1Min, sendAmount0Min, account.address, deadline]
        sendValue = sendAmount0
      }
      if (token1.address === 'AVAX') {
        func = 'addLiquidityAVAX'
        params = [token0.address, pair.stable, sendAmount0, sendAmount0Min, sendAmount1Min, account.address, deadline]
        sendValue = sendAmount1
      }
  ...
  stakeLiquidity = async (payload) => {
    try {
      ...
      this.emitter.emit(ACTIONS.TX_ADDED, {
        title: `Stake ${pair.symbol} in the gauge`, type: 'Liquidity', verb: 'Liquidity Staked', transactions: [
          {
            uuid: stakeAllowanceTXID,
...
//alert(pair.gauge.address)
  ...
  addLiquidityAndStake = async (payload) => {
    try {
      ...
      this.emitter.emit(ACTIONS.TX_ADDED, {
        title: `Add liquidity to ${pair.symbol}`, type: 'Liquidity', verb: 'Liquidity Added', transactions: [
          {
            uuid: allowance0TXID,
            description: `Checking your ${token0.symbol} allowance`,
            status: 'WAITING'
  ...
  _getDepositAllowance = async (web3, token, account) => {
    try {
      ...
      let tt = BigNumber(allowance).div(10 ** token.decimals).toFixed(token.decimals);
  ...
  _getStakeAllowance = async (web3, pair, account) => {
    try {
      ...
      return BigNumber(allowance).div(10 ** pair.decimals).toFixed(6)
    } catch (ex) {
      console.error(ex)
      return null
    }
  ...
  _getWithdrawAllowance = async (web3, pair, account) => {
    try {
      ...
      return parseInt(allowance) == 0 ? 0 : BigNumber(allowance).div(10 ** pair.decimals).toFixed(6)
    } catch (ex) {
      console.error(ex)
      return null
    }
  ...
  quoteAddLiquidity = async (payload) => {
    try {
      ...
      const res = await routerContract.methods.quoteAddLiquidity(addy0, addy1, pair.stable, sendAmount0, sendAmount1).call()
      ...
      const returnVal = {
        ...
        output: BigNumber(res.liquidity).div(10 ** (pair.decimals)).toFixed(6)
      }
  ...
  getLiquidityBalances = async (payload) => {
    try {
      ...
      const returnVal = {
        token0: BigNumber(token0Balance).div(10 ** pair.token0.decimals).toFixed(6),
        token1: BigNumber(token1Balance).div(10 ** pair.token1.decimals).toFixed(6),
  ...
  removeLiquidity = async (payload) => {
    try {
      ...
      this.emitter.emit(ACTIONS.TX_ADDED, {
        title: `Remove liquidity from ${pair.symbol}`, type: 'Liquidity', verb: 'Liquidity Removed', transactions: [
          {
            uuid: allowanceTXID,
            description: `Checking your ${pair.symbol} allowance`,
            status: 'WAITING'
      ...
      const quoteRemove = await routerContract.methods.quoteRemoveLiquidity(token0.address, token1.address, pair.stable, sendAmount).call()
      ...
      this._callContractWait(web3, routerContract, 'removeLiquidity', [token0.address, token1.address, pair.stable, sendAmount, sendAmount0Min, sendAmount1Min, account.address, deadline], account, gasPrice, null, null, withdrawTXID, (err) => {
        if (err) {
          return this.emitter.emit(ACTIONS.ERROR, err)
        }
  ...
  unstakeAndRemoveLiquidity = async (payload) => {
    try {
      ...
      this._callContractWait(web3, gaugeContract, 'withdraw', [sendAmount], account, gasPrice, null, null, unstakeTXID, async (err) => {
        ...
        this._callContractWait(web3, routerContract, 'removeLiquidity', [token0.address, token1.address, pair.stable, balanceOf, sendAmount0Min, sendAmount1Min, account.address, deadline], account, gasPrice, null, null, withdrawTXID, (err) => {
          if (err) {
            return this.emitter.emit(ACTIONS.ERROR, err)
          }
  ...
  unstakeLiquidity = async (payload) => {
    try {
      ...
      this.emitter.emit(ACTIONS.TX_ADDED, {
        title: `Unstake liquidity from gauge`, type: 'Liquidity', verb: 'Liquidity Unstaked', transactions: [
          {
            uuid: unstakeTXID,
            description: `Unstake LP tokens from the gauge`,
            status: 'WAITING'
  ...
  quoteRemoveLiquidity = async (payload) => {
    try {
      ...
      const res = await routerContract.methods.quoteRemoveLiquidity(token0.address, token1.address, pair.stable, sendWithdrawAmount).call()
  ...
  createGauge = async (payload) => {
    try {
      ...
      this.emitter.emit(ACTIONS.TX_ADDED, {
        title: `Create liquidity gauge for ${pair.token0.symbol}/${pair.token1.symbol}`, type: 'Liquidity', verb: 'Gauge Created', transactions: [
          {
            uuid: createGaugeTXID,
            description: `Create gauge`,
            status: 'WAITING'
  ...
  quoteSwap = async (payload) => {
    ...
    try {
      ...
      if((fromAsset.symbol == 'USDC' && toAsset.symbol == 'GLACIER') || (fromAsset.symbol == 'GLACIER' && toAsset.symbol == 'GLACIER')){
        amountOuts.push({
          routes: [{
            from: addy0,
            to: routeAssets[0].address,
            stable: false
          }, {
            from: routeAssets[0].address,
            to: addy1,
            stable: false
          }],
          routeAsset: routeAssets[0]
        })
      }else{
  ...
  swap = async (payload) => {
    try {
      ...
      this.emitter.emit(ACTIONS.TX_ADDED, {
        title: `Swap ${fromAsset.symbol} for ${toAsset.symbol}`, type: 'Swap', verb: 'Swap Successful', transactions: [
          {
            uuid: allowanceTXID,
  ...
  createVest = async (payload) => {
    try {
      ...
      const govToken = this.getStore('govToken')
      let { amount, unlockTime } = payload.content
      if (unlockTime > 7257600) {
        unlockTime = 7257600
      }
      ...
      this.emitter.emit(ACTIONS.TX_ADDED, {
        title: `Vest ${govToken.symbol} until ${unlockString}`, type: 'Vest', verb: 'Vest Created', transactions: [
          {
            uuid: allowanceTXID,
      ...
      this._callContractWait(web3, veTokenContract, 'create_lock', [sendAmount, unlockTime1 + ''], account, gasPrice, null, null, vestTXID, (err) => {
        if (err) {
  ...
  increaseVestAmount = async (payload) => {
    try {
      ...
      this.emitter.emit(ACTIONS.TX_ADDED, {
        title: `Increase vest amount on token #${tokenID}`, type: 'Vest', verb: 'Vest Increased', transactions: [
          {
            uuid: allowanceTXID,
            description: `Checking your ${govToken.symbol} allowance`,
            status: 'WAITING'
  ...
  increaseVestDuration = async (payload) => {
    try {
      ...
      const govToken = this.getStore('govToken')
      let { tokenID, unlockTime } = payload.content
      if (unlockTime > 7257600) {
        unlockTime = 7257600
      }
      // ADD TRNASCTIONS TO TRANSACTION QUEUE DISPLAY
      ...
      this.emitter.emit(ACTIONS.TX_ADDED, {
        title: `Increase unlock time on token #${tokenID}`, type: 'Vest', verb: 'Vest Increased', transactions: [
          {
            uuid: vestTXID,
  ...
  withdrawVest = async (payload) => {
    try {
      ...
      this.emitter.emit(ACTIONS.TX_ADDED, {
        title: `Withdraw vest amount on token #${tokenID}`, type: 'Vest', verb: 'Vest Withdrawn', transactions: [
          {
            uuid: vestTXID,
  ...
  vote = async (payload) => {
    try {
      ...
      this.emitter.emit(ACTIONS.TX_ADDED, {
        title: `Cast vote using token #${tokenID}`, verb: 'Votes Cast', transactions: [
          {
            uuid: voteTXID,
  ...
  createBribe = async (payload) => {
    ...
    try {
      ...
      this.emitter.emit(ACTIONS.TX_ADDED, {
        title: `Create bribe on ${gauge.token0.symbol}/${gauge.token1.symbol}`, verb: 'Bribe Created', transactions: [
          {
            uuid: allowanceTXID,
  ...
  getVestBalances = async (payload) => {
    try {
      ...
      const filteredPairs = pairs.filter((pair) => {
        return pair && pair.gauge.bribeAddress
      })
        ...
        filteredPairs.map(async (pair) => {
          ...
          const bribesEarned = await Promise.all(
            pair.gauge.bribes.map(async (bribe) => {
              ...
              return {
                earned: BigNumber(earned).div(10 ** parseInt(bribe.token.decimals)).toFixed(6),
              }
            })
          )
  ...
  getRewardBalances = async (payload) => {
    try {
      ...
      const { tokenID } = payload.content
      //const  tokenID =1830;
      const pairs = this.getStore('rewardpairs')
      ...
      const filteredPairs3 = [...pairsI.filter((pair) => {
        return pair && pair.gauge.feesAddress
      })]

      for(let i=0;i<filteredPairs.length;i++){
        newThePairsB[i].gauge.reserve0 = filteredPairs[i].gauge.reserve0
        newThePairsB[i].gauge.reserve1 = filteredPairs[i].gauge.reserve1
        newThePairsI[i].gauge.reserve0 = filteredPairs3[i].gauge.reserve0
        newThePairsI[i].gauge.reserve1 = filteredPairs3[i].gauge.reserve1
      }
      ...
      if (tokenID ) {
        ...
        const bribesEarned = await Promise.all(
          newThePairsB.map(async (pair) => {
            ...
            const  bribesEarned1 = await Promise.all(
              pair.gauge.bribes.map(async (bribe) => {
               ...
               console.log(bribesEarned, "bribe")
                ...
                bribe.earned = BigNumber(earned).div(10 ** bribe.token.decimals).toFixed(6)
            ...
            pair.gauge.bribesEarned = bribesEarned1
      ...
      let FeesEarned
       ...
       FeesEarned = await Promise.all(
          newThePairsI.map(async (pair) => {
            
            const FeesEarned = await Promise.all(
              pair.gauge.bribesi.map(async (bribe) => {
        ...
        filteredFeesBribe = FeesEarned.filter((pair) => {
        ...
        if (BigNumber(veDistEarned).gt(0)) {
          veDistReward.push({
            ...
            earned: BigNumber(veDistEarned).div(10 ** govToken.decimals).toFixed(6),
     ...
     // }
       ...
       const rewardsEarned = await Promise.all(
        filteredPairs2.map(async (pair) => {
          ...
          pair.gauge.rewardsEarned = BigNumber(earned).div(10 ** 18).toFixed(6)
          return pair
        })
       )
  ...
  claimBribes = async (payload) => {
    try {
      ...
      this.emitter.emit(ACTIONS.TX_ADDED, {
        title: `Claim rewards for ${pair.token0.symbol}/${pair.token1.symbol}`, verb: 'Rewards Claimed', transactions: [
          {
            uuid: claimTXID,
      ...
      this._callContractWait(web3, gaugesContract, 'claimBribes', [sendGauges, sendTokens, tokenID], account, gasPrice, null, null, claimTXID, async (err) => {
        ...
        this.emitter.emit(ACTIONS.CLAIM_BRIBE_RETURNED)
      })
    } catch (ex) {
      console.error(ex)
      this.emitter.emit(ACTIONS.ERROR, ex)
    ...
    try {
      ...
      this.emitter.emit(ACTIONS.TX_ADDED, {
        title: `Claim rewards for ${pair.token0.symbol}/${pair.token1.symbol}`, verb: 'Rewards Claimed', transactions: [
          {
            uuid: claimTXID,
            description: `Claiming your VoterFees`,
            status: 'WAITING'
          }
        ]
      })
      ...
      const sendTokens = [pair.gauge.feesEarned.map((bribe) => {
        return bribe.token.address
      })]
      ...
      this._callContractWait(web3, gaugesContract, 'claimFees', [sendGauges, sendTokens, tokenID], account, gasPrice, null, null, claimTXID, async (err) => {
        ...
        this.getRewardBalances({ content: { tokenID } })
        this.emitter.emit(ACTIONS.CLAIM_FEES_BRIBE_RETURNED)
      })
    } catch (ex) {
      console.error(ex)
      this.emitter.emit(ACTIONS.ERROR, ex)
  ...
  claimAllRewards = async (payload) => {
    try {
      ...
      let feesBribePairs = pairs.filter((pair) => {
        return pair.rewardType === 'VoterFees'
      })
      ...
      const sendGaugesFee = feesBribePairs.map((pair) => {
        return pair.gauge.feesAddress
      })
      const sendTokensFee = feesBribePairs.map((pair) => {
        return pair.gauge.feesEarned.map((bribe) => {
          return bribe.token.address
        })
      })

      if (bribePairs.length == 0 && feePairs.length == 0 && rewardPairs.length == 0 && rewardPairs.feesBribePairs == 0) {
      ...
      if (feesBribePairs.length > 0) {
        sendOBJ.transactions.push({
          uuid: claimTXID,
          description: `Claiming all your available VoterFees`,
          status: 'WAITING'
        })
      }
      ...
      if (feesBribePairs.length > 0) {
        // SUBMIT CLAIM TRANSACTION FEES
        const gaugesContract = new web3.eth.Contract(CONTRACTS.VOTER_ABI, CONTRACTS.VOTER_ADDRESS)

        const claimPromise = new Promise((resolve, reject) => {
          context._callContractWait(web3, gaugesContract, 'claimFees', [sendGaugesFee, sendTokensFee, tokenID], account, gasPrice, null, null, claimTXID, (err) => {
            if (err) {
              reject(err)
              return
            }
  ...
  claimVeDist = async (payload) => {
    try {
      ...
      this.emitter.emit(ACTIONS.TX_ADDED, {
        title: `Claim distribution for NFT #${tokenID}`, verb: 'Rewards Claimed', transactions: [
          {
            uuid: claimTXID,
  ...
  claimPairFees = async (payload) => {
    try {
      ...
      this.emitter.emit(ACTIONS.TX_ADDED, {
        title: `Claim fees for ${pair.token0.symbol}/${pair.token1.symbol}`, verb: 'Fees Claimed', transactions: [
          {
            uuid: claimTXID,
  ...
  whitelistToken = async (payload) => {
    try {
      ...
      this.emitter.emit(ACTIONS.TX_ADDED, {
        title: `WHITELIST ${token.symbol}`, verb: 'Token Whitelisted', transactions: [
          {
            uuid: whitelistTXID,
      ...
      this._callContractWait(web3, voterContract, 'whitelist', [token.address], account, gasPrice, null, null, whitelistTXID, async (err) => {
        if (err) {
          return this.emitter.emit(ACTIONS.ERROR, err)
        }
  ...
  _callContractWait = async (web3, contract, method, params, account, gasPrice, dispatchEvent, dispatchContent, uuid, callback, paddGasCost, sendValue = 0) => {

    console.log(web3, contract, method, params, account, gasPrice, dispatchEvent, dispatchContent, uuid, callback, paddGasCost, sendValue)
    ...
    const gasCost = await contract.methods[method](...params)
      ...
      .then((gasAmount) => {
        ...
        console.log({ gasAmount, sendValue, gasAmount, sendGasAmount, sendGasPrice })
        contract.methods[method](...params)
          .send({
            nonce: count,
            from: account.address,
            gasPrice: web3.utils.toWei(sendGasPrice, 'gwei'),//web3.utils.toWei(sendGasPrice, 'gwei'),
            gas: gasAmount,//sendGasAmount,
            value: sendValue || 0,
          ...
          .on("error", function (error) {
            console.log(error.toString(), "////", error.toString().includes("-32601"))
  ...
  _callContractWait1 = async (web3,pairContract, contract, method, params, account, gasPrice, dispatchEvent, dispatchContent, uuid, callback, paddGasCost, sendValue = null) => {
    ...
    const paircon = await pairContract.methods.balanceOf(account.address).call();
    ...
    //  const gasCost = contract.methods.deposit(params[0],params[1])
    //  let gasLimit = await gasCost.estimateGas({ from: account.address, value: sendValue })
    //     .then((gasAmount) => {
    //      conosle.log(gasAmount)
    //     })
    //     const context = this
    //     let sendGasAmount = BigNumber(gasAmount).times(1.5).toFixed(0)
    //     let sendGasPrice = BigNumber(gasPrice).times(1.5).toFixed(0)
